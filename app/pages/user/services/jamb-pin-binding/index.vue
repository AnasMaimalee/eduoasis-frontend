...
const downloadFile = async (jobId: string) => {
  ...
  try {
    const response = await $api.raw(
      `/services/jamb-pin-binding/${jobId}/download`,
      {
        method: 'GET',
        responseType: 'blob',
      }
    ...
    let filename = `jamb-pin-binding-${Date.now()}`
...
const fetchRequests = async () => {
  try {
    ...
    const res = await $api('/services/jamb-pin-binding/my', { 
      method: 'GET',
      params 
    })
...
const SERVICE_NAME = 'Jamb PIN Binding request'
...
const submitRequest = async () => {
  try {
    ...
    await $api('/services/jamb-pin-binding', {
      method: 'POST',
      body: form.value
    })
