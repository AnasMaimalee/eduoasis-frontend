...
definePageMeta({
  ...
  middleware: 'auth',
  roles: ['superadmin'],
  title: 'JAMB Admission Checking Status'
})
...
const handleApprove = async () => {
  ...
  try {
    await $api(
      `/services/jamb-admission-status/${currentApproveId.value}/approve`,
      { method: 'POST' }
    )
  ...
  <div class="p-4 md:p-6 space-y-4 bg-emerald-50">
    ...
    <Card class="rounded-t-2xl overflow-hidden">
      ...
      <Table
        ...
        row-key="id"
        :scroll="{ x: 1600, y: 520 }"
        class="status-table text-xs md:text-sm"
      >
        ...
        <!-- File Download -->
         <template #fileCell="{ record }">
           <Button
            ...
            downloadFile({
              id: record.id,
              url: `/services/jamb-admission-status/${record.id}/download`,
