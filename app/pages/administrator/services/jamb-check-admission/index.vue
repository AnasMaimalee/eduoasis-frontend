...
// SERVICE SLUG
// ---------------------------
const serviceSlug = 'jamb-admission-status'
...
onMounted(async () => {
  ...
  refreshTimer = setInterval(() => fetchJobs(activeTab.value), 30_000)
})
        ...
        <Tabs.TabPane key="pending" :tab="`🕒 Pending (${pendingJobs.length})`">
          ...
          >
            ...
            <Table.Column title="Action" width="140" align="center">
              <template #default="{ record }">
                <Button type="primary" size="small" :loading="loading.take[record.id]" @click="takeJob(record)">
                  Take Job
                </Button>
              </template>
            </Table.Column>
