...
const fetchPending = async () => {
  ...
  try {
    const res = await $api('/services/jamb-upload-status/pending')
...
const fetchMyJobs = async () => {
  loading.myJobs = true
  try {
    const res = await $api('/services/jamb-upload-status/my-pending-job')
...
const fetchCompleted = async () => {
  ...
  try {
    const res = await $api('/services/jamb-upload-status/administrator')
...
const takeJob = async (job: any) => {
  ...
  try {
    ...
    await $api(`/services/jamb-upload-status/${job.id}/take`, {
...
const completeJob = async () => {
  ...
  try {
    await $api(`/services/jamb-upload-status/${selectedJob.value.id}/complete`, {
      method: 'POST',
      body: fd
    })
              ...
              <template #default="{ record }">
               <a
                ...
                @click.prevent="
                  viewFile(
                    record.id,
                    `/services/jamb-upload-status/${record.id}/download`
                  )
                "
              >
