...
// SERVICE SLUG
// ---------------------------
const serviceSlug = 'jamb-upload-status'
      ...
      <Tabs v-model:activeKey="activeTab" class="!rounded-2xl overflow-hidden">
        ...
        <Tabs.TabPane :tab="`✅ Completed (${completedJobs.length})`" key="completed">
          ...
          >
            ...
            <Table.Column title="Status" width="120" align="center">
              <template #default="{ record }">
                <Tag :color="record.status === 'approved' ? 'green' : 'red'">{{ record.status?.toUpperCase() }}</Tag>
              </template>
            </Table.Column>
            ...
            <Table.Column title="Result File" width="140" align="center">
              <template #default="{ record }">
                <a href="#" @click.prevent="viewFile(record.id, `/services/${serviceSlug}/${record.id}/download`)">📄 View</a>
              </template>
            </Table.Column>
