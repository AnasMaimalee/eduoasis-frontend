...
const {
  ...
  completeJob,
  beforeUpload
} = useServiceJobs('jamb-upload-status')
...
<template>
  <div class="p-6 space-y-8 bg-emerald-50">
    <!-- HEADER - IDENTICAL TO JAMB RESULT -->
    ...
    <Tabs v-model:activeKey="activeTab">
      <!-- PENDING - ServiceJobsTable handles all columns automatically -->
        ...
        >
          ...
          <Table.Column title="Result File" width="140" align="center">
            <template #default="{ record }">
              <a
                href="#"
                @click.prevent="viewFile(record.id, `/services/jamb-upload-status/${record.id}/download`)"
              >
                📄 View
              </a>
            </template>
    ...
    <!-- UPLOAD MODAL - SAME AS ALL OTHER PAGES -->
    <UploadResultModal
      ...
      serviceLabel="Upload Status Result"
