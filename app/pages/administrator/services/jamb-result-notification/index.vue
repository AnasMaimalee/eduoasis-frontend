...
definePageMeta({
  ...
  middleware: 'auth',
  roles: ['administrator'],
  title: 'JAMB Admission Result Notification'
})

const { viewFile } = useFileViewer()

// ✅ SAME composable usage as JAMB Result - just different service slug
const {
  pendingJobs,
  processingJobs,
  completedJobs,
  ...
  completeJob,
  beforeUpload
} = useServiceJobs('jamb-admission-result-notification')
...
<template>
  <div class="p-6 space-y-8 bg-emerald-50">
    <!-- HEADER - EXACT SAME AS JAMB RESULT -->
    ...
    <Tabs v-model:activeKey="activeTab">
      ...
      <!-- COMPLETED TAB - UPDATED ENDPOINTS -->
        ...
        >
          ...
          <Table.Column title="Result File" width="140" align="center">
            <template #default="{ record }">
              <a
                href="#"
                @click.prevent="viewFile(record.id, `/services/jamb-admission-result-notification/${record.id}/download`)"
              >
                📄 View
              </a>
            </template>
    ...
    <!-- MODAL - EXACT SAME AS JAMB RESULT -->
    <UploadResultModal
      ...
      serviceLabel="Admission Result Notification"
...
<style scoped>
/* EXACT SAME STYLES AS JAMB RESULT */
