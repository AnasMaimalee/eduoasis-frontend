...
const fetchPending = async () => {
  ...
  try {
    const res = await $api('/services/jamb-result/pending')
...
const fetchMyJobs = async () => {
  loading.myJobs = true
  try {
    const res = await $api('/services/jamb-result/my-pending-job')
...
const fetchCompleted = async () => {
  ...
  try {
    const res = await $api('/services/jamb-result/administrator')
...
const takeJob = async (job: any) => {
  ...
  try {
    ...
    await $api(`/services/jamb-result/${job.id}/take`, {
...
const completeJob = async () => {
  ...
  try {
    await $api(`/services/jamb-result/${selectedJob.value.id}/complete`, {
      method: 'POST',
      body: fd
    })
              ...
              <template #default="{ record }">
                <a
                  ...
                  @click.prevent="
                    viewFile(
                      record.id,
                      `/services/jamb-result/${record.id}/download`
                    )
                  "
                >
