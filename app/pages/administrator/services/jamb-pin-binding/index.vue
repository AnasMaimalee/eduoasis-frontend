...
const {
  ...
  completeJob,
  beforeUpload
} = useServiceJobs('jamb-pin-binding')
...
<template>
  <div class="p-6 space-y-8 bg-emerald-50">
    ...
    <Tabs v-model:activeKey="activeTab">
      <!-- PENDING - ServiceJobsTable handles PIN Binding columns -->
        ...
        >
          ...
          <Table.Column title="Result File" width="140" align="center">
            <template #default="{ record }">
              <a
                href="#"
                @click.prevent="viewFile(record.id, `/services/jamb-pin-binding/${record.id}/download`)"
              >
                📄 View
              </a>
            </template>
    ...
    <!-- UPLOAD MODAL - SAME AS JAMB RESULT -->
    <UploadResultModal
      ...
      serviceLabel="PIN Binding Result"
