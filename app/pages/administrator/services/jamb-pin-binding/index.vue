...
const fetchPending = async () => {
  ...
  try {
    const res = await $api('/services/jamb-pin-binding/pending')
...
const fetchMyJobs = async () => {
  loading.myJobs = true
  try {
    const res = await $api('/services/jamb-pin-binding/my-pending-job')
...
const fetchCompleted = async () => {
  ...
  try {
    const res = await $api('/services/jamb-pin-binding/administrator')
...
const takeJob = async (job: any) => {
  ...
  try {
    ...
    await $api(`/services/jamb-pin-binding/${job.id}/take`, {
...
const completeJob = async () => {
  ...
  try {
    await $api(`/services/jamb-pin-binding/${selectedJob.value.id}/complete`, {
      method: 'POST',
      body: fd
    })
      ...
      <template #default="{ record }">
        <a
          ...
          @click.prevent="
            viewFile(
              record.id,
              `/services/jamb-pin-binding/${record.id}/download`
            )
          "
        >
